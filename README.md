...
- **多版式 ✓** 内置 4 种版式模板（双栏对照/均分网格/上图下文/分类列表），录入时选，`parse.py` 按选择返回布局树、`render.py` 泛化绘制。D3 的零 Key 轻量替代
- D3：见下方「模块与替换路线」
...
| F3 版式解析 | `services/parse.py` | **多版式模板**（`services/templates.py`，录入选择） | **D3** 有参考图时接真识别覆盖模板 |
